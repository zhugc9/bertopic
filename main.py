...
def validate_config(config_path: Path = None) -> bool:
    ...
    try:
        ...
        # 基本结构验证 - 适配新的用户友好配置格式
        required_sections = ['output_settings']  # 移除不存在的topic_parameters
        optional_sections = ['data_files', 'topic_settings', 'analysis_features']
        ...
        # 检查至少有一个可选的关键配置段
        has_optional = any(section in config for section in optional_sections)
        if not has_optional:
            print(f"❌ 配置至少需要包含以下一个部分: {', '.join(optional_sections)}")
            return False
...
def check_data_files() -> bool:
    ...
    try:
        ...
        if not media_data and not social_data:
            friendly_paths = config.get('data_files', {})
    ...
    try:
        ...
        if loaded_data:
            ...
            if optimization_results:
                ...
                save_tuning_results(str(translator.user_config_path), optimization_results)
                success = True
            else:
                success = False
        else:
