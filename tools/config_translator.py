...
class ConfigTranslator:
    ...
    def _translate_results_paths(self) -> Dict[str, Any]:
        ...
        file_names = self.user_config.get('output_settings', {}).get('file_names', {})

        topic_summary_name = file_names.get('topic_summary', '主题摘要表.csv')
        document_mapping_name = file_names.get('document_mapping', '文档主题分布表.csv')
        keyword_table_name = file_names.get('keyword_table', '主题关键词表.csv')
        analysis_report_name = file_names.get('analysis_report', '主题分析报告.txt')

        return {
            'output_dir': output_dir,
            'model_dir': f"{output_dir}/训练模型",
            'summary_file': f"{output_dir}/{topic_summary_name}",
            'summary_enhanced': f"{output_dir}/增强版主题摘要.csv",
            'cross_lingual_file': f"{output_dir}/跨语言构成报告.csv",
            'evolution_file': f"{output_dir}/主题演化分析.csv",
            'viz_file': f"{output_dir}/主题可视化.html",
            'source_analysis': f"{output_dir}/来源主题热力图.html",
            'timeline_analysis': f"{output_dir}/主题时间演化图.html",
            'frame_heatmap': f"{output_dir}/框架热力图.html",
            'analysis_summary': f"{output_dir}/分析结果摘要.txt",
            'charts_summary': f"{output_dir}/图表清单.txt",
            'document_topic_mapping': f"{output_dir}/{document_mapping_name}",
            'keyword_table': f"{output_dir}/{keyword_table_name}",
            'analysis_report': f"{output_dir}/{analysis_report_name}"
        }
    ...
    def _translate_visualization(self) -> Dict[str, Any]:
        ...
        mode_config = self.user_config.get('analysis_mode', {})
        if isinstance(mode_config, dict):
            enable_sota = mode_config.get('enable_academic_visualizations', True)
        else:
            enable_sota = True
    ...
    def _translate_optimization(self) -> Dict[str, Any]:
        ...
        tuning_config = self.user_config.get('auto_tuning', {})
        
        # 尝试从高级配置读取搜索空间
        advanced_tuning = self.user_config.get('auto_tuning_advanced', {})
        search_ranges = advanced_tuning.get('search_space', {})
        
        return {
            'enable': enable_opt,
            'n_trials': tuning_config.get('trials', 100),
            
            'search_space': {
                'n_neighbors': search_ranges.get('topic_granularity', [5, 50]),
                'n_components': [2, 10],
                'min_dist': [0.0, 0.5],
                'min_cluster_size': search_ranges.get('topic_granularity', [5, 50]),
                'min_samples': search_ranges.get('clustering_sensitivity', [2, 15]),
                'ngram_range': [1, 4],
