...
def load_topics_summary(file_path) -> pd.DataFrame:
    ...
    if not file_path.exists():
        ...
        print("请先运行 main.py 生成 主题摘要表.csv 文件")
        sys.exit(1)
...
def main():
    ...
    keyword_filename = results_cfg.get('names', {}).get('keyword_table', '主题关键词表.csv')
    input_file = results_dir / keyword_filename
    ...
    # 保存结果
    labeled_filename = keyword_filename.replace('.csv', '_带AI标签.csv') if keyword_filename.endswith('.csv') else "主题关键词表_带AI标签.csv"
