...
class TopicCoherenceEvaluator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if isinstance(config, dict):
            self.sample_size = (
                config.get('hyperparameter_optimization', {})
                .get('sample_size')
            )
        self.progress_callback = None
        self._progress_threads: Dict[str, threading.Thread] = {}
        self._progress_flags: Dict[str, bool] = {}
    ...
    def _start_periodic_updates(self, message: str, lock_name: str, interval: float = 5.0):
        if self._progress_flags.get(lock_name):
            return
        self._progress_flags[lock_name] = True
        self._progress_meta[lock_name] = {
            'message': message,
            'interval': interval,
        ...
        def _worker():
            while self._progress_flags.get(lock_name):
                meta = self._progress_meta.get(lock_name)
                if meta and self.progress_callback:
                    meta['counter'] += 1
                    elapsed = time.time() - meta['start_time']
                    composed = (
                        f"{meta['message']} | 第 {meta['counter']} 次提示 | 累计 {elapsed:.1f} 秒"
                    )
                    self.progress_callback(composed)
                time.sleep(interval)

        thread = threading.Thread(target=_worker, daemon=True)
        thread.start()
        self._progress_threads[lock_name] = thread

    def _stop_periodic_updates(self, lock_name: str):
        if not self._progress_flags.get(lock_name):
            return
        self._progress_flags[lock_name] = False
        meta = self._progress_meta.get(lock_name)
        if meta and self.progress_callback:
            elapsed = time.time() - meta.get('start_time', time.time())
            self.progress_callback(
                f"{meta['message']} | 已停止 | 总提示 {meta['counter']} 次 | 用时 {elapsed:.1f} 秒"
            )
        thread = self._progress_threads.get(lock_name)
        if thread and thread.is_alive():
            thread.join(timeout=0.2)
        self._progress_threads.pop(lock_name, None)
        self._progress_meta.pop(lock_name, None)
    ...
    def calculate_coherence_score(self, 
        ...
        try:
            ...
            self._notify(f"有效主题数量: {len(valid_topics)}")
            
            if len(valid_topics) < 2:
                self._notify("有效主题 < 2，直接返回默认分数")
                return 0.1
            ...
            for topic_id in valid_topics:
                ...
                if words:
                    top_terms = [word for word, _ in words[:10] if isinstance(word, str) and word.strip()]
                    if top_terms:
                        topic_words.append(top_terms)
            
            if not topic_words:
                self._notify("未取到有效主题词，返回默认分数")
                return 0.1
            ...
            if self.sample_size and self.sample_size > 0:
                max_docs = min(self.sample_size, max_docs)
            self._notify(f"准备分词文档，采样数量: {max_docs}")
            for idx, doc in enumerate(documents[:max_docs]):
                if doc and isinstance(doc, str):
                    cleaned = doc
                    if '{' in cleaned and '}' in cleaned and ':' in cleaned:
                        continue
                    tokens = self.tokenizer.tokenize_text(cleaned)
                    filtered_tokens = [
                        word for word in tokens
                        if len(word) > 2 and not any(ch in word for ch in '{}[]:"')
                    ]
                    if filtered_tokens:
                        texts.append(filtered_tokens)
                if (idx + 1) % 100 == 0 and (idx + 1) != max_docs:
                    self._notify(
                        f"已处理 {idx + 1} / {max_docs} 篇文档"
                    )
            
            if not texts:
                self._notify("分词结果为空，返回默认分数")
                return 0.1
            
            # 创建词典和语料库
            self._notify("构建词典和语料库")
            self._start_periodic_updates("构建词典/语料库进行中...", lock_name="dictionary_build", interval=5.0)
            dictionary = Dictionary(texts)
            corpus = [dictionary.doc2bow(text) for text in texts]
            self._stop_periodic_updates("dictionary_build")
            
            # 计算一致性
            self._notify("调用 gensim CoherenceModel 计算一致性")
            self._start_periodic_updates("gensim 一致性计算进行中...", lock_name="coherence_calc", interval=5.0)
            ...
            self._stop_periodic_updates("coherence_calc")
            self._notify(f"一致性分数 ({coherence_type}): {coherence_score:.4f}")
            ...
            if self.progress_callback:
                self.progress_callback(f"一致性评估异常：{e}")
...
class OptunaBERTopicOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.coherence_evaluator.set_progress_callback(self._announce)
    ...
    def create_objective_function(self, 
        ...
        self.logger.info(f"预计算文档嵌入向量（{total_docs}个文档）...")
        self._announce(f"共加载 {total_docs} 条文档，开始预计算句向量")
        ...
        self._announce(f"句向量预计算完成，用时 {embed_time:.1f} 秒")
        ...
        def objective(trial) -> float:
            ...
            try:
                ...
                progress_prefix = f"试验 {trial.number + 1}"
                if self.total_trials:
                    progress_prefix += f"/{self.total_trials}"
                ...
                self._announce(f"{progress_prefix}：模型训练完成，用时 {train_time:.1f} 秒")
                
                # 计算主题一致性分数
                coherence_start = time.time()
                self.coherence_evaluator._start_periodic_updates(
                    f"{progress_prefix}：一致性评估进行中...",
                    lock_name=f"trial_{trial.number}_coherence",
                    interval=10.0
                )
                coherence_score = self.coherence_evaluator.calculate_coherence_score(
                    topic_model, documents, 'c_v'
                )
                self.coherence_evaluator._stop_periodic_updates(
                    lock_name=f"trial_{trial.number}_coherence"
                )
                coherence_time = time.time() - coherence_start
                self._announce(f"{progress_prefix}：一致性评估完成，得分 {coherence_score:.4f}，用时 {coherence_time:.1f} 秒")
                ...
                self.logger.info(f"试验 {trial.number}: 一致性={coherence_score:.4f}, "
                               ...
                               f"参数={hyperparams}")
                progress_msg = f"{progress_prefix}：完成，综合得分 {final_score:.4f}"
                self._announce(progress_msg)
                ...
                error_msg = (
                    f"试验 {trial.number + 1}" +
                    (f"/{self.total_trials}" if self.total_trials else "") +
                    f" 失败：{e}"
                )
                self._announce(error_msg, level="error")
    ...
    def optimize_hyperparameters(self, 
        ...
        self._announce(f"即将运行 {n_trials} 次试验，正在准备 Optuna 任务...")
        ...
        self._announce(f"Optuna 全部试验完成，总耗时 {optimization_time:.1f} 秒")
