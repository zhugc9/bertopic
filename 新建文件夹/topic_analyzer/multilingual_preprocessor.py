...
if __name__ == "__main__":
    ...
    # 测试文档
    test_documents = [
        "人工智能技术正在快速发展，机器学习算法不断改进。",
        "Artificial intelligence is transforming the way we work and live.",
        "Искусственный интеллект меняет наш мир к лучшему.",
        "深度学习模型在计算机视觉领域取得了重大突破。",
        "Natural language processing enables better human-computer interaction.",
        "Машинное обучение помогает решать сложные задачи."
    ]
    
    print("测试多语言预处理器...")
    
    # 测试语言分析
    stats = analyze_document_languages(test_documents, test_config)
    print(f"语言分布: {stats}")
    
    # 测试向量化器
    vectorizer = create_multilingual_vectorizer(test_config)
    print("✅ 多语言向量化器创建成功")
    
    # 测试分词
    tokenizer = MultilingualTokenizer(test_config)
    for doc in test_documents[:3]:
        tokens = tokenizer.tokenize_text(doc)
        print(f"文档: {doc[:30]}...")
