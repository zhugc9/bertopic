...
class DataLoader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        """
        self.config = config
        data_paths = config.get('data_paths', {}) or config.get('data_files', {})
        self.media_path = data_paths.get('media_data') or data_paths.get('traditional_media')
        self.social_media_path = data_paths.get('social_media_data') or data_paths.get('social_media')
        processing = config.get('data_processing', {})
        self.text_column = processing.get('text_column', 'Unit_Text')
        self.metadata_columns = processing.get('metadata_columns', [])
        self.merge_strategy = processing.get('merge_strategy', 'concat')
    ...
    def load_and_prepare_data(self) -> Tuple[List[str], pd.DataFrame]:
        ...
        logger.info("开始加载数据...")
        dataframes = []
        if self.media_path and self.media_path != "null" and Path(self.media_path).exists():
            media_df = self._load_excel(self.media_path, "媒体数据")
            media_df['data_source'] = 'media'
            dataframes.append(media_df)
        if self.social_media_path and self.social_media_path != "null" and Path(self.social_media_path).exists():
            social_df = self._load_excel(self.social_media_path, "社交媒体数据")
            social_df['data_source'] = 'social_media'
            dataframes.append(social_df)
        if not dataframes:
            raise FileNotFoundError("未找到任何有效的数据文件")
        if len(dataframes) == 1:
            combined_df = dataframes[0]
            logger.info(f"使用单数据源: {len(combined_df)} 条记录")
        else:
            combined_df = self._merge_dataframes(dataframes[0], dataframes[1])
    ...
    def _merge_dataframes(self, media_df: pd.DataFrame, social_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.merge_strategy == 'concat':
            combined_df = pd.concat([media_df, social_df], ignore_index=True, sort=False)
        else:
            combined_df = pd.concat([media_df, social_df], ignore_index=True, sort=False)
        return combined_df
    ...
    def _extract_documents_and_metadata(self, df: pd.DataFrame) -> Tuple[List[str], pd.DataFrame]:
...
class DataLoader:
    ...
    def load_and_prepare_data(self) -> Tuple[List[str], pd.DataFrame]:
        ...
        # 尝试加载社交媒体数据
        if self.social_media_path and Path(self.social_media_path).exists():
            social_df = self._load_excel(self.social_media_path, "社交媒体数据")
            social_df['data_source'] = 'social_media'
            dataframes.append(social_df)
