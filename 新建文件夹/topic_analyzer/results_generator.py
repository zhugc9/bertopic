...
class ResultsGenerator:
    ...
    def save_document_topic_mapping(self, documents: List[str], topics: List[int], metadata_df: Optional[pd.DataFrame] = None):
        """保存文档主题映射表"""
        try:
            # 创建基础映射表
            mapping_df = pd.DataFrame({
                '文档序号': range(len(documents)),
                '文档内容': [doc[:100] + '...' if len(doc) > 100 else doc for doc in documents],
                '主题编号': topics,
                '主题标签': [f"主题 {t}" if t != -1 else "离群点" for t in topics]
            })
            
            # 如果有元数据，添加相关列
            if metadata_df is not None and len(metadata_df) == len(documents):
                # 添加常见的元数据列
                for col in ['日期', 'Source', 'Incident']:
                    if col in metadata_df.columns:
                        mapping_df[col] = metadata_df[col].values
            
            # 保存为CSV
            mapping_path = Path(self.results_paths['document_topic_mapping'])
            mapping_path.parent.mkdir(parents=True, exist_ok=True)
            mapping_df.to_csv(mapping_path, index=False, encoding='utf-8-sig')
            logger.info(f"  ✓ 文档主题映射已保存: {mapping_path}")
            
        except Exception as e:
            logger.warning(f"  ⚠ 文档主题映射保存失败: {e}")
    
    def generate_topic_visualization(self, topic_model: BERTopic, documents: List[str], topics: Optional[List[int]] = None):
        """生成主题可视化"""
        try:
            # 生成主题距离图
            ...
            try:
                ...
                fallback = visualizer.plot_topic_distance_map(
                    topic_model,
                    filename_prefix="topic_distance_map"
                )
                if fallback:
                    logger.info("  ✓ 已使用备用方式生成主题距离图（8-主题距离分布图）")
            except Exception as inner:
                logger.error(f"  ❌ 备用可视化生成失败: {inner}")
