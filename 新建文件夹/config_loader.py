...
DEFAULT_USER_CONFIG: Dict[str, Any] = {
    ...
    'advanced': {
        ...
        'trials': 100,
        'save_best': 5,
        'sample_ratio_stage1': 0.1,
        'min_samples_stage1': 300,
        'stage1_repeats': 1,
        'sample_ratio_stage2': 0.3,
        'min_samples_stage2': 800,
        'top_n_refine': 5,
    ...
    'ai_labeling_advanced': {
        ...
        'prompt_templates': {
            'academic': '基于关键词：{keywords}，生成{length}的{style}中文标签。输出：{"topic_label": "标签"}'
        },
    },
...
def _build_results_paths(user_config: Dict[str, Any]) -> Dict[str, str]:
    ...
    return {
        ...
        'summary_file': path_for('topic_summary', '1-主题摘要表.csv'),
        'summary_enhanced': path_for('keyword_table', '增强版主题摘要.csv'),
        'cross_lingual_file': str(Path(results_dir) / '3-跨语言构成报告.csv'),
        'evolution_file': str(Path(results_dir) / '主题演化分析.csv'),
        'viz_file': str(Path(results_dir) / '6-主题可视化.html'),
        ...
        'document_topic_mapping': path_for('document_mapping', '2-主题文档详细分布.csv'),
        'keyword_table': path_for('keyword_table', '主题关键词表.csv'),
        'analysis_report': path_for('analysis_report', '4-主题分析报告.txt'),
    }
...
def build_runtime_config(user_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    runtime['analysis'] = {
        'time_analysis': {
            ...
            'evolution_tuning': features.get('time_evolution', {}).get('evolution_tuning', True),
            'global_tuning': features.get('time_evolution', {}).get('global_tuning', True),
        },
        'source_analysis': {
    ...
    auto_tuning_advanced = user_config.get('advanced', {}).get('auto_tuning_advanced', {})
    search_space = auto_tuning_advanced.get('search_space', {})
    ...
    runtime['hyperparameter_optimization'] = {
        ...
        'sample_ratio_stage1': auto_tuning.get('sample_ratio_stage1', 0.1),
        'min_samples_stage1': auto_tuning.get('min_samples_stage1', 300),
        'stage1_repeats': auto_tuning.get('stage1_repeats', 1),
        'sample_ratio_stage2': auto_tuning.get('sample_ratio_stage2', 0.3),
        'min_samples_stage2': auto_tuning.get('min_samples_stage2', 800),
        'top_n_refine': auto_tuning.get('top_n_refine', 5),
        'coherence_weight': auto_tuning.get('coherence_weight', 0.7),
        'diversity_weight': auto_tuning.get('diversity_weight', 0.3),
        'max_tokens': auto_tuning.get('max_tokens', 2500),
        'max_doc_length': auto_tuning.get('max_doc_length', 300),
        'timeout_seconds': auto_tuning.get('timeout_seconds', 300),
        # 搜索空间配置（完全从用户配置读取）
        'search_space': {
            'n_neighbors': search_space.get('n_neighbors', [5, 50]),
            'n_components': search_space.get('n_components', [2, 10]),
            'min_dist': search_space.get('min_dist', [0.0, 0.5]),
            'min_cluster_size': search_space.get('min_cluster_size', [10, 150]),
            'min_samples': search_space.get('min_samples', [1, 15]),
            'metric': search_space.get('metric', ['cosine', 'euclidean']),
            'cluster_selection_method': search_space.get('cluster_selection_method', ['eom', 'leaf']),
            'max_features': search_space.get('max_features', [None, 1000, 5000, 10000]),
        },
        # 得分权重配置
        'score_weights': auto_tuning_advanced.get('score_weights', {
            'noise_weight': 0.5,
            'diversity_weight': 0.3,
            'uniformity_weight': 0.2,
        }),
