...
def build_prompt(keywords: str, config: dict) -> str:
    """构建API请求的提示词"""
    ai_config = config['ai_labeling']
    label_settings = ai_config.get('label_settings', {})
    
    # 获取提示词风格和模板
    prompt_style = ai_config.get('prompt_style', 'academic')
...
def call_llm_api(prompt: str, config: dict) -> str:
    """调用大语言模型API"""
    ai_config = config['ai_labeling']
    api_config = ai_config['api_config']
    
    # 使用与您其他项目一致的API配置格式
    api_key = api_config['API_KEYS'][0] if api_config['API_KEYS'] else ""
...
def main():
    ...
    # 检查API密钥
    api_config = ai_config.get('api_config', {})
    ...
    if not api_keys or not api_keys[0] or api_keys[0] == "YOUR_API_KEY_HERE":
        ...
        print("位置: ai_labeling.api_config.API_KEYS")
        return
    
    # 加载数据
    results_dir = PROJECT_ROOT / config['output_settings']['results_folder']
    input_file = results_dir / "主题关键词表.csv"
    ...
    # 保存结果
    output_file = results_dir / "主题关键词表_带AI标签.csv"
