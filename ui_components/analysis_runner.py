...
class AnalysisRunner:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        self.user_config_path = Path("config.yaml")

        if config is None:
            self.config = self._load_translated_config()
        else:
            self.config = config

        self.results_dir = Path(self.config.get('results_paths', {}).get('output_dir', 'results'))
        self.results_dir.mkdir(exist_ok=True)

    def _load_translated_config(self) -> Dict[str, Any]:
        if not self.user_config_path.exists():
            return {}

        try:
            translator = ConfigTranslator(str(self.user_config_path))
    ...
    def _load_data(self):
        ...
        config_ready = bool(self.config)

        if not config_ready:
            st.error("❌ 配置加载失败，请检查 config.yaml")
            st.session_state.analysis_running = False
            return
    ...
    def _generate_results(self):
        ...
        # 保存主题摘要
        summary_file = Path(self.config['results_paths']['summary_file'])
        ...
        try:
            ...
            if n_topics > 0 and n_docs == n_topics:
                ...
                doc_mapping_file = Path(self.config['results_paths']['document_topic_mapping'])
                doc_mapping_file.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            # 3. 生成学术级图表
            st.write("  • 生成学术图表...")
            analyzer.visualizer.generate_all_visualizations(
        ...
        st.write("```")
        st.write(f"results/")
        st.write(f"├── {Path(self.config['results_paths']['summary_file']).name}                      # 主题摘要")
        st.write(f"├── {Path(self.config['results_paths']['document_topic_mapping']).name}              # 文档-主题映射")
        st.write(f"├── {Path(self.config['results_paths']['cross_lingual_file']).name}           # 跨语言成分分析")
        st.write(f"├── {Path(self.config['results_paths']['evolution_file']).name}          # 动态演化分析")
        ...
        st.write(f"└── {Path(self.config['results_paths']['model_dir']).name}/                          # 训练好的模型")
